  ...
  public  class TCPTun
    {
      ...
      Dictionary<int,TcpPacket>  sendedTable_server=new Dictionary<int,TcpPacket> ();
	Dictionary<int,TcpPacket>  sendedTable_history_server=new Dictionary<int,TcpPacket> ();

	int clientSequence=int.MinValue;
	...
	Dictionary<int, SendRecord> sendrecordTable=new Dictionary<int, SendRecord>();
	...
	Dictionary<int, IPacket> packetTable_l=new Dictionary<int, IPacket>();

	Dictionary<int, IPacket> packetTable=new Dictionary<int, IPacket>();
	...
	public void process_server(final Packet packet,EthernetHeader ethernetHeader,IpV4Header ipV4Header,TcpPacket tcpPacket,bool client){
		...
		if(!preDataReady){
			if(!connectReady){
				...
				if(!tcpHeader.getSyn()&&tcpHeader.getAck()){
					...
					bool selfAck=selfAckTable.contains(ipV4Header.getIdentification());
	...
	public void process_client(CapEnv capEnv,final Packet packet,EthernetHeader ethernetHeader,IpV4Header ipV4Header,TcpPacket tcpPacket,bool client){
	...
	public void setConnectReady(bool connectReady) {
